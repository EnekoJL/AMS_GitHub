...
void DWT_Init(void) {
  //DWT->LAR = 0xC5ACCE55;
  if (!(CoreDebug->DEMCR &
        CoreDebug_DEMCR_TRCENA_Msk)) {               // Modified by PissedOff
    CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk;  // Enable Trace Off
    //DWT->LAR = 0xC5ACCE55;                           // Modified by Pissed
    DWT->CYCCNT = 0;
...
*/
// DWT INIT NUEVO FUNCIONAL
void DWT_Init(void) {
  CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk;  // Habilitar acceso al DWT
  DWT->CTRL |= 1;   // Habilitar el contador de ciclos (bit 0)
  DWT->CYCCNT = 0;  // Resetear el contador

  // printf("DWT->CTRL after enabling: 0x%08lX\n", DWT->CTRL);
}
